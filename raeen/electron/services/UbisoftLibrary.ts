...
export class UbisoftLibrary {
    ...
    async getInstalledGames(): Promise<UbisoftGame[]> {
    ...
    private async scanRegistry(): Promise<UbisoftGame[]> {
        const games: UbisoftGame[] = [];
        ...
        try {
            ...
            for (const key of keys) {
                try {
                    ...
                    if (id && installPath) {
                        const title = path.basename(installPath.replace(/\\$/, ''));
                        games.push({
                            id: id,
                            title: title,
    ...
    private async scanDrives(): Promise<UbisoftGame[]> {
        const games: UbisoftGame[] = [];
        const drives = await this.getDrives();

        for (const drive of drives) {
            const commonPaths = [
                path.join(drive, 'Ubisoft Game Launcher', 'games'),
