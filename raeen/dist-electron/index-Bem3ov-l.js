...
const externalDataInterceptor = require("./externalDataInterceptor-Dw5yVLyv.js");
...
const getValidatedProcessCredentials = (profileName, data, profiles) => {
  ...
  main.clientExports.setCredentialFeature(credentials, "CREDENTIALS_PROCESS", "w");
  return credentials;
};
const resolveProcessCredentials = async (profileName, profiles, logger) => {
  ...
  if (profiles[profileName]) {
    ...
    if (credentialProcess !== void 0) {
      const execPromise = require$$0.promisify(externalDataInterceptor.externalDataInterceptor?.getTokenRecord?.().exec ?? require$$1.exec);
      try {
