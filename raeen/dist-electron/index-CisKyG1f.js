...
const parseKnownFiles = require("./parseKnownFiles-rM0Sd9bV.js");
const resolveCredentialSource = (credentialSource, profileName, logger) => {
  const sourceProvidersMap = {
    EcsContainer: async (options) => {
      const { fromHttp } = await Promise.resolve().then(() => require("./index-DhDziYqu.js"));
      const { fromContainerMetadata } = await Promise.resolve().then(() => require("./index-Cr0T3qDJ.js"));
    ...
    Ec2InstanceMetadata: async (options) => {
      ...
      const { fromInstanceMetadata } = await Promise.resolve().then(() => require("./index-Cr0T3qDJ.js"));
      return async () => fromInstanceMetadata(options)().then(setNamedProvider);
    },
    Environment: async (options) => {
      logger?.debug("@aws-sdk/credential-provider-ini - credential_source is Environment");
      const { fromEnv } = await Promise.resolve().then(() => require("./index-5YPf7XwV.js"));
...
const setNamedProvider = (creds) => main.clientExports.setCredentialFeature(creds, "CREDENTIALS_PROFILE_NAMED_PROVIDER", "p");
...
const resolveAssumeRoleCredentials = async (profileName, profiles, options, visitedProfiles = {}, resolveProfileData2) => {
  ...
  if (!options.roleAssumer) {
    const { getDefaultRoleAssumer } = await Promise.resolve().then(() => require("./index-DZiWiDX4.js")).then((n) => n.index);
  ...
  if (isCredentialSourceWithoutRoleArn(profileData)) {
    return sourceCredsProvider.then((creds) => main.clientExports.setCredentialFeature(creds, "CREDENTIALS_PROFILE_SOURCE_PROFILE", "o"));
  } else {
    const params = {
    ...
    return options.roleAssumer(sourceCreds, params).then((creds) => main.clientExports.setCredentialFeature(creds, "CREDENTIALS_PROFILE_SOURCE_PROFILE", "o"));
  }
};
...
class LoginCredentialsFetcher {
  ...
  async refresh(token) {
    const { SigninClient, CreateOAuth2TokenCommand } = await Promise.resolve().then(() => require("./index-BEOidmtY.js")).then((n) => n.index);
  ...
  async loadToken() {
    ...
    try {
      let tokenData;
      try {
        tokenData = await main.readFile(tokenFilePath, { ignoreCache: this.init?.ignoreCache });
      } catch {
...
const fromLoginCredentials = (init) => async ({ callerClientConfig } = {}) => {
  ...
  return main.clientExports.setCredentialFeature(credentials, "CREDENTIALS_LOGIN", "AD");
};
...
const resolveLoginCredentials = async (profileName, options) => {
  ...
  return main.clientExports.setCredentialFeature(credentials, "CREDENTIALS_PROFILE_LOGIN", "AC");
};
const isProcessProfile = (arg) => Boolean(arg) && typeof arg === "object" && typeof arg.credential_process === "string";
const resolveProcessCredentials = async (options, profile) => Promise.resolve().then(() => require("./index-BOfoG0Dd.js")).then(({ fromProcess }) => fromProcess({
  ...options,
  profile
})().then((creds) => main.clientExports.setCredentialFeature(creds, "CREDENTIALS_PROFILE_PROCESS", "v")));
const resolveSsoCredentials = async (profile, profileData, options = {}) => {
  const { fromSSO } = await Promise.resolve().then(() => require("./index-CiFFe6rS.js"));
  return fromSSO({
    profile,
    ...
    if (profileData.sso_session) {
      return main.clientExports.setCredentialFeature(creds, "CREDENTIALS_PROFILE_SSO", "r");
    } else {
      return main.clientExports.setCredentialFeature(creds, "CREDENTIALS_PROFILE_SSO_LEGACY", "t");
    }
  });
};
...
const resolveStaticCredentials = async (profile, options) => {
  ...
  return main.clientExports.setCredentialFeature(credentials, "CREDENTIALS_PROFILE", "n");
};
...
const resolveWebIdentityCredentials = async (profile, options) => Promise.resolve().then(() => require("./index-iUve4ROd.js")).then(({ fromTokenFile }) => fromTokenFile({
  ...
  logger: options.logger,
  parentClientConfig: options.parentClientConfig
})().then((creds) => main.clientExports.setCredentialFeature(creds, "CREDENTIALS_PROFILE_STS_WEB_ID_TOKEN", "q")));
