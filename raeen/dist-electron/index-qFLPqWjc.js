...
const fs = require("fs");
const getSSOTokenFromFile = require("./getSSOTokenFromFile-BCJHgLzP.js");
...
const getSsoOidcClient = async (ssoRegion, init = {}) => {
  const { SSOOIDCClient } = await Promise.resolve().then(() => require("./index-BcOSi9CQ.js")).then((n) => n.index);
...
const getNewSsoOidcToken = async (ssoToken, ssoRegion, init = {}) => {
  const { CreateTokenCommand } = await Promise.resolve().then(() => require("./index-BcOSi9CQ.js")).then((n) => n.index);
...
const resolveSSOCredentials = async ({ ssoStartUrl, ssoSession, ssoAccountId, ssoRegion, ssoRoleName, ssoClient, clientConfig, parentClientConfig, profile, filepath, configFilepath, ignoreCache, logger }) => {
  ...
  const { SSOClient, GetRoleCredentialsCommand } = await Promise.resolve().then(() => require("./loadSso-C4dEURCo.js"));
