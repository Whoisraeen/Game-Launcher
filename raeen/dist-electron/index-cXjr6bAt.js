...
const fs = require("fs");
const getSSOTokenFromFile = require("./getSSOTokenFromFile-CNUOTC1i.js");
...
const getSsoOidcClient = async (ssoRegion, init = {}) => {
  var _a;
  const { SSOOIDCClient } = await Promise.resolve().then(() => require("./index-CEo-2fFS.js"));
  const coalesce = (prop) => {
    var _a2, _b;
...
const getNewSsoOidcToken = async (ssoToken, ssoRegion, init = {}) => {
  const { CreateTokenCommand } = await Promise.resolve().then(() => require("./index-CEo-2fFS.js"));
...
const resolveSSOCredentials = async ({ ssoStartUrl, ssoSession, ssoAccountId, ssoRegion, ssoRoleName, ssoClient, clientConfig, parentClientConfig, profile, filepath, configFilepath, ignoreCache, logger }) => {
  ...
  const { SSOClient, GetRoleCredentialsCommand } = await Promise.resolve().then(() => require("./loadSso-CEAnQf53.js"));
