...
const fs = require("fs");
const getSSOTokenFromFile = require("./getSSOTokenFromFile-D8fkrkio.js");
...
const getSsoOidcClient = async (ssoRegion, init = {}) => {
  const { SSOOIDCClient } = await Promise.resolve().then(() => require("./index-ZKPYY0ab.js")).then((n) => n.index);
...
const getNewSsoOidcToken = async (ssoToken, ssoRegion, init = {}) => {
  const { CreateTokenCommand } = await Promise.resolve().then(() => require("./index-ZKPYY0ab.js")).then((n) => n.index);
...
const resolveSSOCredentials = async ({ ssoStartUrl, ssoSession, ssoAccountId, ssoRegion, ssoRoleName, ssoClient, clientConfig, parentClientConfig, profile, filepath, configFilepath, ignoreCache, logger }) => {
  ...
  const { SSOClient, GetRoleCredentialsCommand } = await Promise.resolve().then(() => require("./loadSso-eszAOu9p.js"));
