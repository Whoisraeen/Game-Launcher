...
const fs = require("fs");
const getSSOTokenFromFile = require("./getSSOTokenFromFile-B3CCvlSw.js");
const parseKnownFiles = require("./parseKnownFiles-Dt1k4iur.js");
class TokenProviderError extends main.ProviderError {
...
const getSsoOidcClient = async (ssoRegion, init = {}) => {
  var _a;
  const { SSOOIDCClient } = await Promise.resolve().then(() => require("./index-7WofT4jZ.js"));
  const coalesce = (prop) => {
    var _a2, _b;
...
const getNewSsoOidcToken = async (ssoToken, ssoRegion, init = {}) => {
  const { CreateTokenCommand } = await Promise.resolve().then(() => require("./index-7WofT4jZ.js"));
...
const resolveSSOCredentials = async ({ ssoStartUrl, ssoSession, ssoAccountId, ssoRegion, ssoRoleName, ssoClient, clientConfig, parentClientConfig, profile, filepath, configFilepath, ignoreCache, logger }) => {
  ...
  const { SSOClient, GetRoleCredentialsCommand } = await Promise.resolve().then(() => require("./loadSso-BDEbl1Vh.js"));
  ...
  if (ssoSession) {
    main.setCredentialFeature(credentials, "CREDENTIALS_SSO", "s");
  } else {
    main.setCredentialFeature(credentials, "CREDENTIALS_SSO_LEGACY", "u");
  }
  return credentials;
};
