...
const main = require("./main-B5F_ScUS.js");
...
const resolveHttpAuthSchemeConfig = (config) => {
  const config_0 = main.distCjsExports.resolveAwsSdkSigV4Config(config);
...
const getRuntimeConfig$1 = (config) => {
  return {
    ...
    base64Decoder: config?.base64Decoder ?? main.fromBase64,
    base64Encoder: config?.base64Encoder ?? main.toBase64,
    ...
    httpAuthSchemes: config?.httpAuthSchemes ?? [
      {
        ...
        signer: new main.distCjsExports.AwsSdkSigV4Signer()
      },
      {
        schemeId: "smithy.api#noAuth",
        identityProvider: (ipc) => ipc.getIdentityProvider("smithy.api#noAuth") || (async () => ({})),
        signer: new main.distCjsExports$1.NoAuthSigner()
      }
    ],
    logger: config?.logger ?? new main.NoOpLogger(),
    protocol: config?.protocol ?? new main.protocolsExports.AwsRestJsonProtocol({ defaultNamespace: "com.amazonaws.sso" }),
    serviceId: config?.serviceId ?? "SSO",
    urlParser: config?.urlParser ?? main.parseUrl,
    utf8Decoder: config?.utf8Decoder ?? main.fromUtf8,
    utf8Encoder: config?.utf8Encoder ?? main.toUtf8
  };
};
const getRuntimeConfig = (config) => {
  ...
  main.distCjsExports.emitWarningIfUnsupportedVersion(process.version);
  const loaderConfig = {
  ...
  return {
    ...
    runtime: "node",
    defaultsMode,
    authSchemePreference: config?.authSchemePreference ?? main.loadConfig(main.distCjsExports.NODE_AUTH_SCHEME_PREFERENCE_OPTIONS, loaderConfig),
    bodyLengthChecker: config?.bodyLengthChecker ?? main.calculateBodyLength,
    defaultUserAgentProvider: config?.defaultUserAgentProvider ?? main.createDefaultUserAgentProvider({ serviceId: clientSharedValues.serviceId, clientVersion: packageInfo.version }),
    maxAttempts: config?.maxAttempts ?? main.loadConfig(main.NODE_MAX_ATTEMPT_CONFIG_OPTIONS, config),
    region: config?.region ?? main.loadConfig(main.NODE_REGION_CONFIG_OPTIONS, { ...main.NODE_REGION_CONFIG_FILE_OPTIONS, ...loaderConfig }),
    requestHandler: main.NodeHttpHandler.create(config?.requestHandler ?? defaultConfigProvider),
    retryMode: config?.retryMode ?? main.loadConfig({
      ...main.NODE_RETRY_MODE_CONFIG_OPTIONS,
      default: async () => (await defaultConfigProvider()).retryMode || main.DEFAULT_RETRY_MODE
    }, config),
    sha256: config?.sha256 ?? main.Hash.bind(null, "sha256"),
    streamCollector: config?.streamCollector ?? main.streamCollector,
    useDualstackEndpoint: config?.useDualstackEndpoint ?? main.loadConfig(main.NODE_USE_DUALSTACK_ENDPOINT_CONFIG_OPTIONS, loaderConfig),
    useFipsEndpoint: config?.useFipsEndpoint ?? main.loadConfig(main.NODE_USE_FIPS_ENDPOINT_CONFIG_OPTIONS, loaderConfig),
    userAgentAppId: config?.userAgentAppId ?? main.loadConfig(main.NODE_APP_ID_CONFIG_OPTIONS, loaderConfig)
  };
};
...
class SSOClient extends main.Client {
  ...
  constructor(...[configuration]) {
    ...
    this.middlewareStack.use(main.schemaExports.getSchemaSerdePlugin(this.config));
    ...
    this.middlewareStack.use(main.distCjsExports$1.getHttpAuthSchemeEndpointRuleSetPlugin(this.config, {
      httpAuthSchemeParametersProvider: defaultSSOHttpAuthSchemeParametersProvider,
      identityProviderConfigProvider: async (config) => new main.distCjsExports$1.DefaultIdentityProviderConfig({
        "aws.auth#sigv4": config.credentials
      })
    }));
    this.middlewareStack.use(main.distCjsExports$1.getHttpSigningPlugin(this.config));
  }
  destroy() {
    super.destroy();
  }
...
main.schemaExports.TypeRegistry.for(n0).registerError(InvalidRequestException2, InvalidRequestException$1);
...
main.schemaExports.TypeRegistry.for(n0).registerError(ResourceNotFoundException2, ResourceNotFoundException$1);
var RoleCredentials = [
  3,
  n0,
  _RC,
...
main.schemaExports.TypeRegistry.for(n0).registerError(TooManyRequestsException2, TooManyRequestsException$1);
...
main.schemaExports.TypeRegistry.for(n0).registerError(UnauthorizedException2, UnauthorizedException$1);
var SSOServiceException2 = [-3, _s, "SSOServiceException", 0, [], []];
main.schemaExports.TypeRegistry.for(_s).registerError(SSOServiceException2, SSOServiceException$1);
