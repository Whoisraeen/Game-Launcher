...
const fs = require("fs");
const getSSOTokenFromFile = require("./getSSOTokenFromFile-CJIrYXDI.js");
...
const getSsoOidcClient = async (ssoRegion, init = {}) => {
  var _a;
  const { SSOOIDCClient } = await Promise.resolve().then(() => require("./index-dFw7LOs_.js"));
  const coalesce = (prop) => {
    var _a2, _b;
...
const getNewSsoOidcToken = async (ssoToken, ssoRegion, init = {}) => {
  const { CreateTokenCommand } = await Promise.resolve().then(() => require("./index-dFw7LOs_.js"));
...
const resolveSSOCredentials = async ({ ssoStartUrl, ssoSession, ssoAccountId, ssoRegion, ssoRoleName, ssoClient, clientConfig, parentClientConfig, profile, filepath, configFilepath, ignoreCache, logger }) => {
  ...
  const { SSOClient, GetRoleCredentialsCommand } = await Promise.resolve().then(() => require("./loadSso-Jwthg7WN.js"));
